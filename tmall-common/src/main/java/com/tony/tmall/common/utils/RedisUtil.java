package com.tony.tmall.common.utils;
...
public class RedisUtil {
   ...
   /* public String getAndSet(String key, String value) {
    ...
    public <T> List<T> multiGetObjs(Collection<String> keys){
    ...
    public Long lLeftPushAllLong(String key, Collection<Long> value) {
        return redisTemplate.opsForList().leftPushAll(key, value);
    }
    /**
     * 当list存在的时候才加入
    ...
    public void delBigHash(String bigHashKey) {
        ...
        while(true){
            Cursor<Map.Entry<Object, Object>> entryCursor = this.hScan(bigHashKey, scanOptions);
            boolean flag = false;
            if(entryCursor!=null){
                while (entryCursor.hasNext()) {
                    Map.Entry<Object, Object> next = entryCursor.next();
                    this.hDelete(bigHashKey,next.getKey());
                    flag = true;
                }
            }
            if(!flag){
    ...
    public void delBigList(String bigListKey) {
        Long len = this.lLen(bigListKey);
        int counter = 0;
        int left = 100;
        while(counter<len){
            this.lTrim(bigListKey,left,len);
            counter+=left;
        }
        this.delete(bigListKey);
    }
    /**
     *    删除bigkey
     * @param bigSetKey
    ...
    public void delBigSet(String bigSetKey) {
        ...
        ScanOptions scanOptions = ScanOptions.scanOptions().count(100).build();
        while(true){
            Cursor<Object> stringCursor = this.sScan(bigSetKey, scanOptions);
            boolean flag = false;
            if(stringCursor!=null){
                while (stringCursor.hasNext()) {
                    Object next = stringCursor.next();
                    this.sRemove(bigSetKey,next);
                    flag = true;
                }
            }
            if(!flag){
    ...
    public void delBigZSet(String bigZsetKey) {
        ...
        ScanOptions scanOptions = ScanOptions.scanOptions().count(100).build();
        while(true){
            Cursor<ZSetOperations.TypedTuple<Object>> typedTupleCursor = this.zScan(bigZsetKey, scanOptions);
            boolean flag = false;
            if(typedTupleCursor!=null){
                while (typedTupleCursor.hasNext()) {
                    ZSetOperations.TypedTuple<Object> next = typedTupleCursor.next();
                    this.zRemove(bigZsetKey,next.getValue());
                    flag = true;
                }
            }
            if(!flag){
